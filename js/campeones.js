...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NUEVOS ELEMENTOS DEL MEDIA CONTAINER ---
    const coverMediaContainer = document.getElementById('cover-media-container');
    const coverVideo = document.getElementById('cover-video');
    const playOverlay = document.getElementById('play-overlay');

    if (coverMediaContainer && coverVideo && playOverlay) {
        // Al darle clic a la imagen, se expande
        coverMediaContainer.addEventListener('click', (e) => {
            if (!coverMediaContainer.classList.contains('expanded')) {
                coverMediaContainer.classList.add('expanded');
            }
        });

        // Al darle clic al ícono de play
        playOverlay.addEventListener('click', (e) => {
            e.stopPropagation(); // Evita que el contenedor reciba el clic
            
            // Entrar en pantalla completa
            if (coverVideo.requestFullscreen) {
                coverVideo.requestFullscreen();
            } else if (coverVideo.webkitRequestFullscreen) {
                coverVideo.webkitRequestFullscreen();
            } else if (coverVideo.msRequestFullscreen) {
                coverVideo.msRequestFullscreen();
            }
            
            coverVideo.play();
        });

        // Cuando el usuario pausa o termina el video, salimos de la pantalla completa enseguida
        const closeFullscreenAndPause = () => {
            ...
            } else if (coverVideo.webkitDisplayingFullscreen || coverVideo.webkitSupportsFullscreen) {
                // Específico para algunos Safari/iOS
                if (coverVideo.webkitExitFullscreen) {
                    coverVideo.webkitExitFullscreen();
                }
            }
        };

        coverVideo.addEventListener('pause', closeFullscreenAndPause);
        coverVideo.addEventListener('ended', closeFullscreenAndPause);

        // Si el usuario sale de la pantalla completa por cualquier medio (Esc, botón nativo, o evento arriba), 
        // pausamos bien todo y contraemos el contenedor.
        const handleFullscreenChange = () => {
            const isFullScreen = document.fullscreenElement || document.webkitFullscreenElement || document.msFullscreenElement || coverVideo.webkitDisplayingFullscreen;
            if (!isFullScreen) {
                coverVideo.pause();
                coverMediaContainer.classList.remove('expanded');
            }
        };
        ...
        // Evento extra vital para dispositivos Apple/Safari sobre el elemento de video directamente
        coverVideo.addEventListener('webkitendfullscreen', () => {
            coverVideo.pause();
            coverMediaContainer.classList.remove('expanded');
        });
    }
    ...
    function populateRanking(type) {
        ...
        rankedData.forEach((item, index) => {
            ...
            rankItem.className = `ranking-item ${topClass}`;
